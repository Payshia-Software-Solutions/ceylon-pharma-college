...
function Certificate() {
  ...
  const fetchSearchResults = async (query) => {
    ...
    try {
      const response = await fetch(`${config.API_BASE_URL}/users/search/${query}`);
      const data = await response.json();
  ...
  const handleNameCardClick = async (username) => {
    try {
      ...
      if (userData?.userEnrollment) {
        const courseDetailsPromises = userData.userEnrollment.map((enrollment) =>
          fetch(`${COURSE_DETAILS_URL}${enrollment.course_code}`).then((response) => response.json())
        );

        const courses = await Promise.all(courseDetailsPromises);

        const enrichedEnrollments = userData.userEnrollment.map((enrollment, index) => ({
          ...enrollment,
          course_name: courses[index]?.course_name || "N/A",
        }));

        setSelectedUserDetails({ ...userData, userEnrollment: enrichedEnrollments });
  ...
  const handleCourseClick = (courseCode) => {
    ...
    setSelectedCourseDetails(courseResult || { courseCode, status: "Not Submitted" });
  ...
  return (
    <div className="bg-maincolor text-white h-auto min-h-[25rem] p-4 flex flex-col items-center justify-center py-12 lg:py-20">
      <div className="mb-8 text-center">
        <h1 className="text-xl md:text-6xl font-bold">Certificate Verification</h1>
        <p className="opacity-90 text-lg">Secure, Fast, and Reliable: Verify your certificates with ease through our online platform.</p>
      </div>
      ...
      <div className="bg-white my-6 px-2 py-2 w-full max-w-xl rounded-lg bg-opacity-40">
        ...
        {!selectedUserDetails && !selectedCourseDetails ? (
          ...
          showCourses && selectedUserDetails?.userEnrollment && (
            <div className="fade-in">
              ...
              <div className="grid grid-cols-3 gap-4 mt-2 text-md md:text-xl">
                {selectedUserDetails?.userEnrollment?.map((enrollment, index) => (
                  ...
                  >
                    <p className="text-blue-600 font-bold text-center">{enrollment.course_name}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        {showCourseResult && (
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 mr-2" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
              <path fillRule="evenodd" d="M11 3a1 1 0 01.707.293l3 3a1 1 0 01-1.414 1.414L12 5.414V15a1 1 0 11-2 0V5.414L6.707 7.707A1 1 0 015.293 6.293l3-3A1 1 0 0111 3z" clipRule="evenodd" />
            </svg>
            Back to Courses
          </button>
        </div>
      ...
      <div className="flex flex-wrap justify-center gap-8">
        ...
        <div className="flex flex-col items-center bg-white bg-opacity-40 text-white p-6 rounded-lg shadow-lg">
          ...
          <p className="text-lg uppercase font-semibold">Over {courseCount || "Loading..."} Courses</p>
        </div>
      </div>
    </div>
  );
