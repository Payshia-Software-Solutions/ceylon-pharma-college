...
// Mapping slugs to enrollment steps
const enrollmentStepsDataMapping = {
  ...
  // Add more courses with their respective enrollment steps here
  'certificate-course-in-tech': [
    {
      title: "Fill out Application",
      description: "Complete your application form for the Tech Course",
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
        </svg>
    ...
    },
    {
      ...
      description: "Ensure your spot by confirming your enrollment soon",
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
        </svg>
    ...
    {
      title: "Get in Touch",
      description: "For more details, contact us at 071 5 884 884 or 077 59 60 884",
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
        </svg>
...
export default function EnrollmentSteps({ slug }) {
  const [steps, setSteps] = useState([]);

  useEffect(() => {
    // Fetch the enrollment steps based on the slug
    const courseSteps = enrollmentStepsDataMapping[slug];
    if (courseSteps) {
      setSteps(courseSteps);
    } else {
      // If no steps are found, set an empty array or display a fallback message
      setSteps([]);
    }
  }, [slug]);
    ...
    return (
      <section className="mt-16 bg-gradient-to-r from-green-700 to-green-900 text-white rounded-xl p-8 shadow-lg">
        ...
        <div className="text-center">
          <p className="text-xl font-bold text-gray-300">Sorry, we couldn't find any enrollment steps for this course.</p>
        </div>
      </section>
    );
  }
